...
**Estimated monthly change: -$32.85**

| | |
|---|---:|
| Current estimate | $32.85 |
| Proposed estimate | $0.00 |
| Monthly change | -$32.85 |
| — fixed | -$32.85 |
| — usage-based | $0.00 |
| Unknown components | 3 |
| Confidence | UNKNOWN |

<sub>2 added · 1 removed · 0 modified · 0 replaced · 1 unchanged</sub>

**3 cost(s) could not be established.** These are not included in the totals above, and are not zero.

- `stack/DynamoDbEndpoint` `Unsupported` — AWS::EC2::VPCEndpoint is not priced by this version, so any cost it carries is not included in the totals
...
- `stack/S3Endpoint` `Unsupported` — AWS::EC2::VPCEndpoint is not priced by this version, so any cost it carries is not included in the totals
...
|---|---|---:|---:|---:|---|
| pull\-request\-cost\-increase | environment=development | $0.00 | — | — | estimate |
...
|---|---|---|---|
| expensive\-development\-change | not matched | — | applies=yes; monthly\_cost\_delta=$\-32\.85; threshold=$100\.00 |
...
| large\-low\-confidence\-change | not matched | — | applies=yes; monthly\_cost\_delta=$\-32\.85; threshold=LOW; confidence=UNKNOWN |
| budget:pull\-request\-cost\-increase:increase | not matched | — | estimated\_delta=$\-32\.85; maximum\_monthly\_increase=$100\.00 |
